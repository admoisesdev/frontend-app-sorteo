...
const EditPage = () => {
	...
	const {
		raffleQueryById: { data },
	} = useRaffle(router.query.slug as string);
	...
	const onSubmit = (values: z.infer<typeof raffleSchema>) => {
		editRaffleMutation.mutate({ body: values, raffleId: router.query.slug as string });
	};
		...
		<Form {...form}>
			<form
				...
				<div className='bg-white/20 backdrop-blur-md rounded-md'>
					<TypographyH1 className='text-center lg:text-4xl'>
						Editar Sorteo
					</TypographyH1>
				</div>
					...
					render={({ field }) => (
						<FormItem>
							...
							<Select
								...
								<SelectContent
									className='bg-blue-dark-app-200/80 
					...
					render={({ field }) => (
						<FormItem className='flex flex-col'>
							...
							<Popover>
								...
								<PopoverContent
									className='w-auto p-0 bg-blue-dark-app-200/80 
								text-white backdrop-blur-md border-0'
									align='start'>
									<Calendar
										mode='single'
